...
# ---------------------------- SEARCH PASSWORD ------------------------------- #
def info(website,username_encoded,password_encoded):
    # website=(base64.b64decode((codecs.encode(website_encoded,'rot_13')).encode())).decode()
    username=(base64.b64decode((codecs.encode(username_encoded,'rot_13')).encode())).decode()
    password=(base64.b64decode((codecs.encode(password_encoded,'rot_13')).encode())).decode()
    info_window=Tk()
...
def search_for_entry():
    ...
    # website_encoded=codecs.encode(((base64.b64encode(website.encode())).decode()),'rot_13')
    if (not website):
...
def add_password():
    ...
    if (not website or not username or not password):
        ...
        # print(choice)
        # website_encoded=codecs.encode(((base64.b64encode(website.encode())).decode()),'rot_13')
        username_encoded=codecs.encode(((base64.b64encode(username.encode()).decode())),'rot_13')
        password_encoded=codecs.encode(((base64.b64encode(password.encode()).decode())),'rot_13')
        if (choice):
            new_data={website:{
                "Email":username_encoded,
